...
def dhash(image, hash_size=8):
    ...
    hashSize (int): The desired hash size (default is 8).
...
def convert_hash(h):
    ...
    Args:
        h (str): The hash to be converted.
...
class MyWindow(QMainWindow):
    ...
    def select_folder_to_index(self):
        ...
        """
        self.index_directory.setText(str(QFileDialog.getExistingDirectory(self, "Select Directory")))

    def image_indexer(self):
        """
        ...
        if os.path.isfile(self.hash_dir.text()):
            hashes.update(pickleloads(open(self.hash_dir.text(),"rb").read()))
        else:
            self.log_box_append("Hashes file not found. Creating new one")
            pickledump(hashes, open(self.hash_dir.text(), "wb"))
        ...
        self.index_status.setText("[INFO] serializing hashes...")
    ...
    def dropEvent(self, event):
        ...
        if event.mimeData().html():
            ...
            self.file_path = str(event.mimeData().urls()[0].toLocalFile())
